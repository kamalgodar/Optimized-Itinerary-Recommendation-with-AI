...
def get_first_30_attractions():
    ...
    url='https://www.tripadvisor.com/Attractions-g293889-Activities-a_allAttractions.true-Nepal.html'
...
def get_remaining_attractions(page):
    ...
    url=f'https://www.tripadvisor.com/Attractions-g293889-Activities-oa{page}-Nepal.html'
    r = requests.get(url, headers=headers)
    soup = BeautifulSoup(r.text, 'html.parser')
    attractions = soup.find_all('div', {'class': '_392swiRT fQ-rHrKc'})
    for item in attractions:
...
#save to a csv file 
df.to_csv('attractions_of_nepal.csv', index=False)
